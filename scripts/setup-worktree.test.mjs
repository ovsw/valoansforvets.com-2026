...
test("copy preserves existing files and only copies approved env files", async (t) => {
  ...
  for (const dir of ["source/frontend", "target/frontend"]) await mkdir(path.join(root, dir), { recursive: true });
...
test("source symlinks are rejected before any copying", async (t) => {
  const root = await mkdtemp(path.join(tmpdir(), "starter-env-test-"));
  t.after(() => rm(root, { recursive: true, force: true }));
  for (const dir of ["source/frontend", "target"]) await mkdir(path.join(root, dir), { recursive: true });
  await writeFile(path.join(root, "source/frontend/.env.local"), "test");
  await symlink(path.join(root, "source/frontend/.env.local"), path.join(root, "source/frontend/.env.local.link"));
  await rename(path.join(root, "source/frontend/.env.local.link"), path.join(root, "source/frontend/.env.local"));
